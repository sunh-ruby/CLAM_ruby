...
def plot_one_point(x, image,  color=None, radius = 10, labels= None):
    ...
    cv2.circle(image, (int(points[0]), int(points[1])), radius, color, -1)
    if labels:
        cv2.putText(image, labels, (int(points[0]), int(points[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
